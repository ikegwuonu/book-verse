import { Column } from "@/components/CustomTable";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  DropdownMenuTrigger,
} from "@/components/ui/dropdown-menu";
import { adminRoutes } from "@/lib/routes";
import { IGetMaterial, IGetTextBook } from "@/lib/types";
import { Download, Edit, Eye, MoreHorizontal, Trash2 } from "lucide-react";
import Image from "next/image";
import Link from "next/link";
import { Timestamp } from "firebase/firestore"; // Ensure this matches your Firebase setup
import { useModal } from "@/zustand/modalStore";

import { convertTimestamp } from "@/lib/utils";
import ViewModal from "./View";
import EditModal from "./Edit";
import DeleteModal from "./Delete";

const statusBadgeColors: Record<string, string> = {
  published: "bg-green-100 text-green-800 hover:bg-green-200",
  draft: "bg-amber-100 text-amber-800 hover:bg-amber-200",
  review: "bg-blue-100 text-blue-800 hover:bg-blue-200",
};
const formatBadgeColors: Record<string, string> = {
  Hardcover: "bg-purple-100 text-purple-800 hover:bg-purple-200",
  Paperback: "bg-indigo-100 text-indigo-800 hover:bg-indigo-200",
  Ebook: "bg-cyan-100 text-cyan-200",
  Audiobook: "bg-pink-100 text-pink-800 hover:bg-pink-200",
};

export const columns = (
  openModal: (type?: string, data?: React.ReactNode) => void
): Column<IGetMaterial & { id: string }>[] => [
  {
    key: "course_code",
    label: "Code",
    render: (item) => (
      <div
        className="truncate font-medium max-w-32 text-xs"
        title={item.course_code}
      >
        {item.course_code}
      </div>
    ),
  },
  {
    key: "topic",
    label: "Topic",
    render: (item) => (
      <>
        <div
          className="truncate font-medium max-w-32 text-xs"
          title={item.topic}
        >
          {item.topic}
        </div>
      </>
    ),
  },
  {
    key: "department",
    label: "Department",
    render: (item) => (
      <div className="max-w-32 text-xs">
        {<p className="truncate">{item.department}</p>}{" "}
        {item.faculty && <p>{item.faculty} </p>}
      </div>
    ),
  },
  {
    key: "lecturer",
    label: "Lecturer",
    render: (item) => (
      <div className="text-xs text-gray-500 mt-1 " title={item.lecturer}>
        <p>{item.lecturer}</p>
        {item.year}
      </div>
    ),
  },

  {
    key: "format",
    label: "Format",
    render: (item) => (
      <Badge
        className={
          formatBadgeColors[item.format] || "bg-gray-100 text-gray-800"
        }
      >
        {item.format}
      </Badge>
    ),
  },
  {
    key: "status",
    label: "Status",
    render: (item) => (
      <>
        <Badge
          className={
            statusBadgeColors[item.status] || "bg-gray-100 text-gray-800"
          }
        >
          {item.status.charAt(0).toUpperCase() + item.status.slice(1)}
        </Badge>
        <p className="max-w-32 text-xs truncate">{item.added_by}</p>
      </>
    ),
  },
  {
    key: "created_at",
    label: "Created At",
    render: (item) => <p>{convertTimestamp(item.created_at)}</p>,
  },
  {
    key: "id",
    label: "Actions",
    render: (item) => (
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" size="icon">
            <MoreHorizontal className="h-4 w-4" />
            <span className="sr-only">Open menu</span>
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="end">
          <DropdownMenuItem asChild>
            <p
              onClick={() => openModal("edit", <ViewModal textbook={item} />)}
              className="flex items-center cursor-pointer"
            >
              <Eye className="h-4 w-4 mr-2" />
              View Details
            </p>
          </DropdownMenuItem>
          <DropdownMenuItem asChild>
            <p
              onClick={() => openModal("edit", <EditModal textbook={item} />)}
              className="flex items-center cursor-pointer"
            >
              <Edit className="h-4 w-4 mr-2" />
              Edit
            </p>
          </DropdownMenuItem>
          <DropdownMenuItem
            className="flex items-center cursor-pointer"
            onClick={() => openModal("edit", <DeleteModal material={item} />)}
          >
            <Download className="h-4 w-4 mr-2" />
            Download
          </DropdownMenuItem>
          <DropdownMenuItem
            onClick={() => openModal("edit", <DeleteModal material={item} />)}
            className="text-red-600 focus:text-red-600 flex items-center cursor-pointer"
          >
            <Trash2 className="h-4 w-4 mr-2" />
            Delete
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    ),
  },
];
